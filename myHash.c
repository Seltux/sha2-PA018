...
//Function to display your hashed message
void displayHash(unsigned char *digest,
    ...
    printf("Your Message H: %s\n", output); 
			  
 }
 
 //Function for test vectors
...
//TEST VECTORS
	...
	static const char myMessageToHash[] = "myMessage"; // Replace "myMessage" by the message that you want to hash.
    sha256((const unsigned char*)myMessageToHash,strlen(myMessageToHash), digest);
    displayHash(digest, SHA256_DIGEST_SIZE);
    sha224((const unsigned char*)myMessageToHash,strlen(myMessageToHash), digest);
    displayHash(digest, SHA224_DIGEST_SIZE);
    sha384((const unsigned char*)myMessageToHash,strlen(myMessageToHash), digest);
    displayHash(digest, SHA384_DIGEST_SIZE);
    sha512((const unsigned char*)myMessageToHash,strlen(myMessageToHash), digest);
    displayHash(digest, SHA512_DIGEST_SIZE);
